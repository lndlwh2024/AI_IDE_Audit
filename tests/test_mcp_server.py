    ...
    async def check():
        ...
        async with stdio_client(params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                assert {t.name for t in tools} == {'audit_changes', 'get_architecture_graph', 'get_ledger_records', 'get_file_diff', 'get_prompts', 'get_project_status', 'pause_project', 'resume_project', 'get_token_usage', 'begin_token_phase', 'finish_token_phase', 'get_operation_usage_log', 'format_audit_report', 'estimate_audit_usage', 'get_final_audit_report', 'get_audit_input_manifest'}
