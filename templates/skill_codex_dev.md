...
## 普通任务的最小同步边界（优先于下文事件示例）

已接入、已授权且当前 A 会话已就绪时，任务开始只调用 `get_sync_updates` 读取该会话游标之后的新账本事件。返回 `events=[]` 就结束增量阅读；不得再读取完整账本、完整图谱或为了同步而列出/读取 Codex 聊天。只有用户这次业务任务本身需要了解页面或代码，才定点读取相应业务文件。用户提及此前阶段但本轮上下文缺少具体范围时，先查项目内的阶段记录；仍缺失时才定点读取当前已知任务中的相关用户要求，不先列出全部聊天，不把聊天历史当协同账本。

纯讨论、方案设计、阅读文件和范围核对，没有实际修改代码或受管理文档时，仅用 `record_prompt` 保存本轮用户原话；不要调用 `record_sync_event`、`complete_sync_operation` 或 `sync_graph` 制造 `analysis_handoff` 等账本事件。需求记录与修改账本是两种数据。只有确实改变了受管理文件，或用户明确要求创建持久交接记录时，才按实际动作记账。无修改时不准备提交，也不启动 B。
...
## 0.4.11 提交前证据检查
开发事件的 git.base_commit 由插件从当前 HEAD 补齐；不要沿用历史测试 SHA，明确填写错编号会被拒绝。prepare_audit_commit 缺少本轮需求或账本即报错，必须在 commit 前修正，不能继续提交再期待 B 补救。普通同步故障重试保留本轮需求；仅明确暂停恢复才归档暂停前需求。已提交且封存缺失的审计不允许改写原证据制造通过，恢复材料单独注明提交后补充及来源。
