...
当前项目已授权且 A 会话已就绪时，任务开始只调用 `get_sync_updates` 获取当前会话游标之后的新账本事件。返回 `events=[]` 就结束增量阅读；不得为 IDE_Audit 再读全量账本、全量图谱或列出/读取 Codex 聊天。业务任务本身需要了解页面或代码时，可定点读取相关业务文件。用户提及前一阶段但本轮上下文缺少范围时，先查项目内阶段记录；仍缺失才定点读取当前已知任务的相关用户要求，不先列出全部聊天。聊天历史不是协同账本。

纯讨论、方案设计、阅读文件和范围核对没有实际修改代码或受管理文档时，只用 `record_prompt` 保存用户原话；不要调用 `record_sync_event`、`complete_sync_operation` 或 `sync_graph` 制造 `analysis_handoff` 等账本事件。需求记录与修改账本是两种数据。确实改变受管理文件，或用户明确要求持久交接时，才按实际动作记账。无修改时不准备提交，也不启动 B。
