...
## 2026-09-14 补充：B 输入分类表与 A/插件职责说明

本节对应本地 0.4.9 输入协议。表中是消息内容的归属，不是多份重复材料。提示词由消息外层提供；事实包使用 audit_one、information、other 三个对象。

| 类别 | 实际字段 | 内容与边界 | Token 统计方式 |
|---|---|---|---|
| 提示词类 | instructions | B 身份、权限、审计职责、判断和展示要求 | 按最终文本粗估 |
| 提示词类 | contract、verdict_schema | 本次请求标识、裁决回收格式 | 按最终文本粗估 |
| 审计一报告类 | audit_one.changed_files | 实际 diff、文件增删改、行号和必要上下文；原始 diff 只发送一次 | 按本次字段粗估 |
| 审计一报告类 | audit_one.diff_summary | 文件数、增删行数汇总 | 按实际结果粗估 |
| 审计一报告类 | audit_one.architecture_signals、signal_summary | 本地规则信号与汇总，不代替 B 判断用户授权 | 按实际结果粗估 |
| 审计一报告类 | audit_one.ledger_verification | 实际 diff 与本轮账本的物理核验差异 | 按实际结果粗估 |
| 审计一报告类 | audit_one.graph_checks | 架构检查发现与范围说明；节点/边增删详情引用图谱变化，不再重复发送 | 按实际结果粗估 |
| 信息类 | information.prompts | 两次提交之间已记录的用户原始请求 | 按原文粗估 |
| 信息类 | information.ledger_events | 本轮原始账本记录，保留原有字段；不重发全部历史，不额外生成解释 | 按实际事件粗估 |
| 信息类 | information.graph_changes | physical/declared 的节点、边增删改，必要前后值和可达上游上下文；node_values 按内容标识共享节点值 | 按生成的变化包粗估 |
| 其他类 | other 中的提交和证据标识 | 当前/父提交、审计标识、时间、提交说明、证据校验标识等 | 按实际内容粗估 |
| 其他类 | other 中的状态、诊断和协议说明 | 分析完整性、缺失证据、协议版本、范围限制 | 按实际内容粗估 |
| 其他类 | JSON 字段名及消息分隔符 | 消息封装开销，单列避免遗漏 | 按最终消息差额计算 |
| 合计 | 完整新增消息 | 以上全部内容之和，不是另加一段内容 | 同时给出分类小计、总计和占比 |

get_audit_input_manifest 与最终报告按实际生成文本给出字符数、字符数/4 至字符数的 token 粗估、按字符近似的占比。没有宿主字段级实测时 actual_tokens=null。不得给每条账本固定 900 token，也不得预设总量再摊分百分比。新增消息估算不包括宿主系统/工具、历史、多次模型请求与输出。完整图谱快照和原始证据仍在本地封存。

### A 与插件：谁维护什么

A 大模型负责理解用户需求、开发、填写实际修改记录的业务内容，并维护模块职责、设计意图和非自动解析的语义关系。插件是本地 Python 程序，负责字段校验、编号、文件锁、追加持久化、已读位置、缓存扫描、原子保存与确定性物理核验。它没有大模型能力，不能凭扫描独立判断业务意图或架构是否获授权。A 调用工具委托机械工作，不意味着把主脑职责交给插件。

1. **账本展示文件**：ledger.jsonl 是原始事实记录；AUDIT_LOG.md 是由记录生成的可读流水，PROJECT_STATE.md 汇总最新记录中的当前状态、后续步骤、问题和风险，便于用户检查与交接。插件只整理已有字段，不调用模型另写业务解释。当前追加事件后全量重建这些派生视图；不是 B 审计必须额外接收的材料，也不是每次 A 都要读的全文。
2. **完整图谱读取**：GraphManager 在程序内载入当前 graph.json，目的是在保留未变化节点、A 已声明语义与依赖的前提下更新，比较候选图谱并安全保存。不是要求 A 模型每次理解全图。常规同步接口返回摘要。人工语义更新接口 update_architecture_graph 当前接收完整图谱对象，不是局部 patch，这是当前接口的限制，不能描述为已全面增量化。
3. **源码扫描**：验证当前文件存在性及 Python 导入、导出等可机械提取的事实，发现增删或外部修改，减少图谱与代码脱节。当前遍历项目路径和文件状态；未变化内容复用缓存，变化 Python 文件读取并解析；结合当前文件索引重新计算依赖。不是每次都把全项目源码发给模型，也不能据此理解完整业务架构。非 Python 语义关系和职责仍依赖 A 的声明。
4. **图谱写入**：当前将候选完整图谱与现有内容比较，无实质变化时不递增版本；变化时原子保存完整 graph.json，追加变更记录并重建 GRAPH_SUMMARY.md。保存全量文件属于本地存储行为，不等于全量模型输入。新增普通说明文档默认不成为新架构节点；已有显式节点保留。
5. **正常 A 流程**：项目授权检查；确需接入/恢复时由 A 调用同步；沿当前协同会话已读位置获取新账本并处理冲突；记录用户原文；加锁及修改；由 A 填写本轮事件，插件追加；图谱机械事实由工具更新，业务语义由 A 维护；提交前保存证据；提交成功后执行审计一并派发 B。没有修改不制造修改事件；常用 complete_sync_operation 当前仍会检查候选图谱，即使最终无需改变图谱版本。
6. **现有限制**：账本底层读取/校验仍全量，向 A 返回才按未读位置分页；新协同会话从第 0 条开始，恢复原会话才能沿用位置。每个工具调用并不把所有中间本地文件返回给模型。程序扫描、落盘、生成 Markdown 本身不消耗模型 token；请求、返回文本和模型后续处理才影响模型用量。本节解释现状，不宣称已修复新会话重复历史或完整语义图谱更新接口。

## 2026-09-14：0.4.10 最终 A 操作逻辑（替代早期全量接管规则）

新 A 和身份/位置未知的 A 只读取最近 2 条账本，少于 2 条则全部读取现有条目；无历史则直接完成。确认返回条目后视为完成接管，已读位置推进到当前末尾，不补读早期历史。这里“已读”表示当前接管流程已完成，不宣称逐条理解历史。原 A 通过项目内持久保存的 Codex 任务 ID 恢复协同会话和位置；插件连接号不是任务身份。任务 ID 由宿主会话提供或 A 传入，无法取得时走最近两条兜底。连续会话的新记录不会被两条上限截断。

| 环节 | A 的工作 | 插件的工作与边界 |
|---|---|---|
| 授权与接入 | 确认当前项目开启 | 未授权/暂停不工作 |
| 接管或恢复 | 新窗口建立当前项目认知；原窗口延续上下文 | 已知任务恢复已读位置；新/未知任务仅最近 2 条 |
| 同步资料 | 阅读新增记录、当前图谱摘要和必要节点 | 本地全量校验账本，只分页返回目标记录；不返回历史全文 |
| 记录需求 | 理解并保存用户原文 | 持久化，不用 A 意图替代用户授权 |
| 编辑 | 处理冲突，修改代码 | 文件锁及续期 |
| 记账 | 填写实际修改记录 | 校验、编号、追加，生成派生展示文件；无修改不造事件 |
| 图谱物理维护 | 调用完成同步操作 | 缓存扫描当前代码，变化才保存图谱；不是提交审计 |
| 图谱语义维护 | 只提交需修改的节点字段/关系 | patch_architecture_graph 校验版本、合并、原子保存；保留未改部分，删除节点清理关联边 |
| 提交前 | 确认提交范围 | 封存需求、账本、图谱及暂存区证据 |
| 提交后 | 查看成功及派发状态 | commit 成功才执行审计一及派发 B |
| B 完成 | 根据需要处理发现 | 保存裁决、完整报告及用量 |

```mermaid
flowchart TD
    A["A 收到任务"] --> B{"项目已授权且未暂停？"}
    B -- 否 --> C["处理授权或恢复，未完成不工作"]
    C --> B
    B -- 是 --> D{"原 A 身份与已读位置可靠？"}
    D -- 是 --> E["恢复原位置，读取新增账本"]
    D -- 否 --> F["读取最近 2 条，确认后完成接管"]
    E --> G["获取当前图谱摘要和必要节点；确需接入/恢复时同步当前状态"]
    F --> G
    G --> H["A 理解需求，保存用户原文，处理冲突并加锁"]
    H --> I["A 修改代码"]
    I --> J{"有实际修改？"}
    J -- 否 --> K["不新增修改事件，释放锁"]
    J -- 是 --> L["A 填写账本，插件追加并检查代码结构图谱"]
    L --> M{"职责或业务关系变化？"}
    M -- 是 --> N["A 提交局部图谱修改；插件校验版本、合并保存"]
    M -- 否 --> O["完成维护，释放锁"]
    N --> O
    O --> P{"本轮提交？"}
    P -- 否 --> Q["保留未提交改动与记录，不启动 B"]
    P -- 是 --> R["保存提交证据，执行 commit"]
    R --> S{"commit 成功？"}
    S -- 否 --> T["处理失败，不启动 B"]
    S -- 是 --> U["插件执行审计一，派发同项目 B"]
    U --> V["B 完成审计二，展示报告及用量"]
```

程序本地全量读取账本、缓存扫描和重建展示文件本身不消耗模型 token，保留当前可靠实现。优先减少模型接收的历史和完整图谱。旧完整图谱更新接口仅兼容保留，A 默认使用局部接口。此次未改动“commit 后才审计”的边界。
...
## 2026-09-19：审计报告可读性修订

审计一固定分为两个小标题：**代码与账本对比**、**架构变化检查**。前者列出真实改动及账本出入；后者列出图谱变化和自动规则的检查范围，不把规则命中当作架构越权结论。账本缺失时必须说明无法核对，不能显示“行范围已核对通过”。先创建后删除、提交中无净变化的文件不能直接称为虚报。

审计二固定分为：**文件修改是否符合需求**、**架构变化是否符合需求**、**代码账本是否如实记录**。每项问题用“发生条件、具体影响、依据、尚未验证部分”解释。正文不使用 P2、AS、QA、PD/DEV 等内部缩写作标题，HIGH、partial、unchanged 等状态改为中文说明。必要的代码名称须说明作用，不堆叠无关审批、环境与测试清单。机器裁决字段保持兼容。

第三部分给出最终裁决及具体后续工作；第四部分保留 A/B 的预测、本次和累计用量，明确预测范围及缺失原因。历史报告只改写表达，不改原始证据和 B 裁决，不启动新一轮模型审计。此次源代码及模板修改不等于旧 B 聊天指令已更新；NEWS 的本次报告单独重新排版交付。

## 2026-09-19 补充：先给结论，仅展示实际用量

审计一、审计二及其子项先给结论，材料不足明确写无法判断；再列具体依据。报告取消所有预测和估算 token 表。实际用量按模型调用环节列输入、缓存输入、输出、推理输出与合计，后续查看报告、答疑单列，不混入原审计。缓存输入和推理输出分别属于输入和输出，不重复计数。宿主未提供内容类别计数时，不伪造图谱、账本或提示词的实测分摊。NEWS 本次历史报告已从原 B 宿主日志核对逐次调用；通用历史用量记录尚无逐次调用数据时须说明缺失，不推算。

## 2026-09-22：普通任务的增量同步与聊天边界

当前 A 会话已就绪时，任务开始只获取该会话游标之后的账本记录。返回零条时结束同步，不为 IDE_Audit 再读全量账本、图谱或聊天。业务任务需要理解页面结构时可定点读取页面源码；需要恢复前一阶段的用户要求时先查本项目阶段记录，仍缺失才定点读取当前已知任务的相关消息，不列出全局聊天。纯讨论/设计没有修改代码或受管理文档时只保存用户原话，不新增修改账本事件，不运行图谱同步或提交审计。只有实际文件变化或用户明确要求持久交接才记对应事件。
