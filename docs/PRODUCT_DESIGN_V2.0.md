...
- 一个插件替代原 Skill 的完整逻辑能力；旧业务历史迁移为插件外工具，不进入产品。
- A 首次接入建图，再次接入立即刷新，成功前不能编辑；图谱只由 A 动态维护。
- B 必须是同项目 Codex 原生大模型任务；正常重入优先复用，桌面活动写入者通过原生消息桥接，同次运行不新建重复 B。归档或确实不存在才创建下一编号；不自动删除聊天，不回放已删除历史。
- 用户接受 B 的宿主读写权限；审计守则要求只读核查和答疑。这替代下文历史设计中对 B 系统级只读沙箱的绝对保证。插件 core 和审计 MCP 仍只读，sync 只维护协同元数据。
- 后台队列与统一 A Skill 协作打开/派发原生 B，插件读取真实 B 新轮次完成校验；A 不代判。跨系统发送不明时保留状态待核对，不盲目重发。

实施版本 0.3.0，验收证据见 HANDOVER、VALIDATION_0.3.0 和 HOST_COMPATIBILITY。后文阶段为产品路线图，不作为当前完成证明。
...
> **实施状态补充（2026-09-09）**：本文定义终局目标，不代表所有能力已经实现。当前 `0.3.0` 已实现本轮确认功能并完成真实 Codex 同项目双轮裁决；完成度与剩余验收门以 `HANDOVER.md`、`DEVELOPMENT_PLAN.md` 和 `DUAL_SYNC_PARITY.md` 为准。
