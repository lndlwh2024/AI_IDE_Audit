...
项目默认关闭，须逐项目明确授权。A/B 可暂停或请求恢复；只有 A 更新当前图谱与同步后重新启用，旧需求与队列不自动重放。B 用量查询使用宿主线程统计或已验证任务归属的日志兼容读取；未知不填零，不包含 A 或账户额度。审计 MCP 的开关及用量记录可写元数据，不提供源码维护工具。NEWS 单向切换流程取代旧回滚方案，工具在插件之外，真实 NEWS 尚未切换。验收以 VALIDATION_0.4.0.md 为准。
