...
def enqueue(root, audit_id, launch=True):
    ...
    with transaction(root, 'queue'):
        if not path.exists():
            atomic_json(path, {'audit_id': audit_id, 'created_at': time.time(), 'status': 'queued'})
    if launch:
        start_worker(root)
...
def status(root, audit_id):
    ...
    for p in metadata_path(root, 'queue').glob('*.json'):
        value = read_json(p)
        if value.get('status') == 'awaiting_desktop':
    ...
    with transaction(root, 'dispatch-worker', timeout=3600):
        ...
        for entry, path in entries:
            if entry['status'] == 'awaiting_desktop':
                return
            if entry['status'] in ('completed', 'needs_attention', 'failed'):
                continue
...
def retry(root, audit_id, manager=None):
    ...
    atomic_json(path, dict(entry, status='queued'))
    start_worker(root)
