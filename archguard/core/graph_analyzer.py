...
def check_cross_module_dependencies(graph: dict, new_edges: list[dict]) -> list[dict]:
    ...
    for edge in new_edges:
        ...
        forbidden = graph.get('forbidden_dependencies', [])
        if from_module and to_module and [from_module, to_module] in forbidden:
            violations.append({
                "from_file": from_node,
                "to_file": to_node,
...
def analyze_graph_changes(diff_result: Any, current_graph: dict, previous_graph: dict = None) -> GraphAnalysisResult:
    ...
    if previous_graph is None:
        raise ValueError('缺少图谱基线，不能将当前全图当作新增拓扑')
    ...
    for node in topo_diff["nodes_added"] + topo_diff["nodes_removed"]:
        mod = nodes_info.get(node, previous_graph.get('nodes', {}).get(node, {})).get("module")
        if mod:
            affected_modules.add(mod)

    for edge in topo_diff['edges_added'] + topo_diff['edges_removed']:
